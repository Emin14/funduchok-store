  ...
  return (
      <div>
          ...
          <div className='user-orders__wrapper'>
            ...
            <div><span>Город доставки: </span><span>{item.city}</span></div>
            <div><span>Статус заказа: </span><span>{item.status}</span></div>
            <div><span>Сумма заказа: </span><span className='user-orders__totalAmount'>{item.orderAmount}</span></div>
            <div><span>Начислено баллов: </span><span className='user-orders__totalAmount'>{item.orderPoint}</span></div>
            <div className='user-orders__products-wrapper' onClick={() => handleClick(index)} >
            ...
            <div style={{display: show === index ? 'block' : 'none'}}>
              {item.products.map((el,ind) => (
                <ul key={`${el.id}-${el.weight}`} >
                  <li><span>{ind+1} </span> <span>{el.title}</span><span> {el.weightTitle}, </span><span>{el.count} шт * </span><span>{el.basePrice} рублей</span></li>
                </ul>
              ))}
            </div>
          </div>

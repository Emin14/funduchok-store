...
export default function PriceTable({packing, handleChange, handleSubmit, currentPackage, count, incrementCount, decrementCount}) {
            ...
            <form id='product' onChange={handleChange} onSubmit={handleSubmit}>
                {packing.map((item, index) => (
                    <label key={item.id} className='priceTable__label'>
                        ...
                        value={item.weight} 
                        data-koef={item.koef} 
                        defaultChecked={item.weight === 0.5 ? true : false}>
                        </input>
                        <span>
                            {item.packingDiscountPrice} руб.
                        </span>
                        <span className='priceTable__weight'>
                            / {item.title}
                        </span> 
                        <span className='priceTable__undiscounted'>
                            {item.packingPrice} руб.
                        </span>
                    </label>
                ))}
            </form>
                <div className='priceTable__discount'>
                    <p> 
                        ...
                        <span className='priceTable__discount_text'>
                            {packing[currentPackage].packingDiscount * count} руб. (10%)
                        </span>
                    </p>
                    <p>До конца акции: </p>
                </div>
                <p  className='priceTable__delivery'>
                    Доставка по Москве в течении 1 дня. Бесплатная доставка при заказе от 4000 руб. по Москве в пределах МКАД.
                </p>
                ...
                <p className='priceTable__discount-info'>
                    ...
                    <span className='priceTable__bonus-note'>
                        {Math.ceil(packing[currentPackage].packingPoint * count)} руб. на бонусный счёт
                    </span> при заказе от 3000 руб. 
                </p>
            </div>
      )
}
